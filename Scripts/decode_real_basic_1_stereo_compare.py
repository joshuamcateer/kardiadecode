...
def decode_ecg_channel_select(fs, audio, resample_rate=600, channel=-1):
    # Select best channel if stereo
...
t_0, rec_0 = decode_ecg_channel_select(SAMPLE_RATE, audio, resample_rate=600, channel=0)
t_1, rec_1 = decode_ecg_channel_select(SAMPLE_RATE, audio, resample_rate=600, channel=1)

plt.figure()
plt.plot(t_0, rec_0, 'b')
plt.plot(t_1, rec_1, 'r')
plt.ylim([-2,6])
plt.show()

plt.figure()
plt.plot(t_0, rec_0-rec_1, 'b')
plt.show()
